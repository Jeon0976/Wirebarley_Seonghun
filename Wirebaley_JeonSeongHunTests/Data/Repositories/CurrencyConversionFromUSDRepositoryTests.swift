...
final class CurrencyConversionFromUSDRepositoryTests: XCTestCase {
    ...
    func test_API요청확인() {
        // given
        let requestDTO = CurrenciesConversionRequestDTO(currencies: "KRW,JPY,PHP", source: "USD")
        ...
        XCTAssertEqual(endpoint.queryParametersEncodable as? CurrenciesConversionRequestDTO, requestDTO)
    }
    ...
    func test_fetch_Currencies_확인() async {
        ...
        mockService.response = CurrenciesConversionResponseFromUSDDTO(source: "USD", quotes: .init(krw: 10.0, jpy: 10.0, php: 10.0))
        let repository = DefaultCurrencyConversionFromUSDRepository(dataTransferService: mockService)
        let expectedDomain = CurrenciesFromUSD(source: "USD", KRW: 10.0, JPY: 10.0, PHP: 10.0)
        
        var result: CurrenciesFromUSD?
        
        // when
        do {
            result = try await repository.fetchCurrencys(currencies: ["KRW", "JPY", "PHP"], source: "USD")
